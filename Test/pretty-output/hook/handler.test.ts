...
describe('pretty-output handler', () => {
    ...
    describe('messageFormatterHandler', () => {
        ...
        it('应该处理微信通道的简单文本', async () => {
            ...
            await messageFormatterHandler(event as any);
            expect(event.context.content).toContain('这是一条测试消息');
        ...
        it('应该处理飞书通道的代码块', async () => {
            ...
            await messageFormatterHandler(event as any);
            expect(event.context.content).toContain('```json');
            expect(event.context.content).toContain('{"key": "value"}');
            expect(event.context.content).toContain('🤖 *OpenClaw AI 助手*');
        });
        ...
        it('应该处理 YAML 格式', async () => {
            ...
            await messageFormatterHandler(event as any);
            expect(event.context.content).toContain('key: value');
            expect(event.context.content).toContain('nested:');
            expect(event.context.content).toContain('item: value');
            expect(event.context.content).toContain('🤖 *OpenClaw AI 助手*');
        });
        ...
        it('应该处理配置文件', async () => {
            ...
            await messageFormatterHandler(event as any);
            expect(event.context.content).toContain('[section]');
            expect(event.context.content).toContain('key=value');
            expect(event.context.content).toContain('🤖 *OpenClaw AI 助手*');
        });
        ...
        it('应该处理多行文本', async () => {
            ...
            await messageFormatterHandler(event as any);
            expect(event.context.content).toContain('第一行');
            expect(event.context.content).toContain('第二行');
            expect(event.context.content).toContain('第三行');
            expect(event.context.content).toContain('🤖 *OpenClaw AI 助手*');
        });
        ...
        it('不应该处理短文本（<500字符）', async () => {
            ...
            await messageFormatterHandler(event as any);
            // 签名会添加，但分隔线不会（因为文本长度<500）
            expect(event.context.content).toContain('🤖 *OpenClaw AI 助手*');
            // 检查内容开头不是分隔线（签名中的 --- 在末尾）
            expect(event.context.content).not.toMatch(/^---\n/);
        });
        ...
        it('应该处理包含列表的内容', async () => {
            ...
            await messageFormatterHandler(event as any);
            expect(event.context.content).toContain('📋 步骤：');
